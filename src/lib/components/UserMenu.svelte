...
<div class="flex items-center justify-between">
	<Menu class="relative">
		...
		<MenuItems
			class={cn(
				'absolute',
				'animate-in data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow',
				'mt-2 w-[180px]'
			)}
		>
			<MenuItem let:active>
			...
			<MenuItem let:active>
				<a
					...
					class={cn(
						'relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
						active ? 'bg-accent text-accent-foreground' : null,
						'inline-flex w-full items-center justify-between text-xs'
					)}
				>
					jianyuan.IO
